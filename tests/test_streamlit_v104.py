...
class StreamlitV104Tests(IsolatedWorldStateTestCase):
    ...
    def test_ai_is_disabled_without_a_user_key_but_other_worlds_work(self):
        ...
        execute = [
            item for item in self.app.button if item.label == "인공지능 질문 실행"
        ][0]
        self.assertTrue(execute.disabled)
        self.navigation().set_value("Library").run()
        self.assertIn("학습 자료실", [item.value for item in self.app.header])
        self.assertFalse(self.app.exception)
    ...
    def test_register_change_and_delete_key_stays_out_of_world_data(self):
        ...
        key_input = [
            ...
            if item.label == "인공지능 연결 키"
        ][0]
        register = [
            item for item in self.app.button if item.label == "연결 키 등록"
        ][0]
        ...
        change = [
            item for item in self.app.button if item.label == "연결 키 변경"
        ][0]
