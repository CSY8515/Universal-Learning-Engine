...
class OfficialUiV107Tests(IsolatedWorldStateTestCase):
    ...
    def test_v1096_restores_original_home_scale_and_dedicated_world_art(self):
        styles = STYLE_PATH.read_text(encoding="utf-8")
        ...
        self.assertNotIn(".ule-world-backdrop::before", styles)
        for index in range(1, 10):
            slug = f"w{index:02d}"
            self.assertIn(
                f".ule-world-backdrop--{slug} {{ background-image: var(--ule-background-{slug}); }}",
                styles,
            )
        self.assertNotIn("--ule-active-theme-image", styles)
        self.assertNotIn("--ule-feature-image", styles)
        self.assertNotIn(".ule-theme-context", styles)
